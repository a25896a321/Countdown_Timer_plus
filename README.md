...
- 影片教學：https://youtu.be/iB75XsHzv-U
- 教學圖（保留舊版原設定、如何新增圖檔.音檔）：https://canva.link/3na9r3dkvlnxvi4
- 自訂新增圖檔 > 請至 Nexon 楓之谷世界資源網站：https://maplestoryworlds.nexon.com/zh-tw/resource
- 自訂新增音檔 > 請至 ttsmaker AI免費語音網站：https://ttsmaker.com/
...
-  [ 龍王設定檔（建議下載完整安裝，此設定圖片、音檔需自行設定） ] https://github.com/a25896a321/Countdown_Timer_plus/releases/download/v1.4.1_config/default.json
...
v1.4.1（2026-05-23）
- 新增正數計時功能，可自行切換（正計時 / 倒計時）
- 允許音效提前x秒，可設為0，於倒數抵達後才響鈴
- 新增"如何使用"說明按鈕
- 新增懸浮窗口模式的啟用/停用按鈕（並新增在懸浮窗設定，可自行決定是否開啟顯示）
- 新增【闇黑龍王】Boss 設定檔
- 搬移 boss、skill、time 資料夾至 other 資料夾內
...
pyinstaller --onefile --windowed --name Countdown_Timer_plus_v1.3.2 \
...
打包後結構：
```
dist/Countdown_Timer_plus_v1.3.2/
├── Countdown_Timer_plus_v1.3.2.exe
├── sound_type/
└── png_type/
```
...
- [x] 修正自動倒數模式時間歸零至下一輪時間有延遲，配置新的時間流逝活及，採用絕對初始時間

### v1.3.1（2026-03-05）
- [x] 修正雪花快捷鍵 insert → Insert
